...
function resaltarNoticiasQueContengan(palabra){
  for (var contador = 0; contador < noticias.length; contador++) {
    noticiaResaltada=noticias[contador];
    if (contienePalabra(noticiaResaltada, palabra)) {
      cambiarColor(noticiaResaltada, "rgb(235, 190, 162)" );
    }
  }
}
...
function resaltarNoticiasQueContengan2(palabra){
  for (var contador = 0; contador < noticias.length; contador++) {
    noticiaResaltada=noticias[contador];
    if (contienePalabra(noticiaResaltada, palabra)) {
      cambiarColor(noticiaResaltada, "rgb(83, 153, 173)" );
    }
  }
}
...
function ocultarNoticiasQueContengan(palabra){
  for (var contador = 0; contador < noticias.length; contador++) {
    noticiaOcultada=noticias[contador];
    if (contienePalabra(noticiaOcultada, palabra)) {
      ocultarNoticia(noticiaOcultada);
    }
  }
}
...
function recortarNoticias(cantPalabras){
  for (var contador = 0; contador < noticias.length; contador++) {
    noticiaRecortada=noticias[contador];
    recortarTexto(noticiaRecortada, cantPalabras);
  }
}
...
resaltarNoticiasQueContengan2("robot");
